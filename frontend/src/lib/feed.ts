...
/** Increment a post's view counter (security-definer RPC, bypasses RLS). */
...
/** Best-effort notification to another user (uses security-definer RPC). */
...
export function subscribeToNewPosts(onPost: (post: Post) => void): () => void {
  const channel = supabase
    .channel(`feed-posts-${Math.random().toString(36).slice(2, 8)}`)
    .on(
      'postgres_changes',
