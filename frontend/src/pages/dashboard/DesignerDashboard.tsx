...
import { Palette, ArrowRight, Briefcase, Users, Sparkles, PenTool } from 'lucide-react'
...
export default function DesignerDashboard() {
  ...
  const role = (profile?.role ?? 'designer') as keyof typeof ROLE_LABELS
  const firstName = profile?.full_name?.split(' ')[0]
  const roleLabel = ROLE_LABELS[role] ?? 'Designer'
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-gray-500">
          Shape products people love — find startups that need your design eye.
        </p>
      </div>

      {/* Unique designer hero */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-fuchsia-600 via-purple-600 to-indigo-700 p-8 text-white">
        <div className="relative z-10">
          ...
          <h2 className="mt-3 text-xl font-bold sm:text-2xl">Your next canvas is one match away</h2>
          <p className="mt-2 max-w-lg text-white/80">
            From brand identity to product UI — FounderHub ranks startups that need your visual craft.
          </p>
          <div className="mt-5 flex flex-wrap gap-3">
            <Link
              to="/explore"
              className="inline-flex items-center gap-2 rounded-lg bg-white px-5 py-2.5 text-sm font-bold text-fuchsia-700 transition-transform hover:scale-105"
            >
              <CompassIcon />
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Link
          ...
          className="rounded-2xl border border-gray-200 bg-white p-5 transition-all hover:border-fuchsia-300 hover:shadow-lg dark:border-dark-300 dark:bg-dark-100"
        >
          <Briefcase className="h-6 w-6 text-fuchsia-500" />
          <p className="mt-3 text-2xl font-bold">{loadingApps ? '—' : applications.length}</p>
          <p className="text-sm text-gray-500">Design applications</p>
        </Link>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <section className="lg:col-span-2">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="font-bold">Recommended for your craft</h2>
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-dark-300 dark:bg-dark-100">
            ...
            {loadingApps ? (
              ...
              <div className="rounded-xl border border-dashed border-gray-300 p-4 text-center dark:border-dark-400">
                <Palette className="mx-auto h-6 w-6 text-gray-400" />
                <p className="mt-2 text-sm text-gray-500">No applications yet — explore and pitch your design.</p>
              </div>
            ) : (
              <div className="flex flex-col gap-3">
                {applications.slice(0, 3).map((app) => (
                  <Link
                    ...
                    className="rounded-xl border border-gray-200 p-3 transition-colors hover:border-fuchsia-300 dark:border-dark-300"
                  >
