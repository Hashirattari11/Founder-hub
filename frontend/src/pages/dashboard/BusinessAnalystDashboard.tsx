...
import { LineChart, ArrowRight, Users, Sparkles, MessagesSquare, Compass } from 'lucide-react'
...
export default function BusinessAnalystDashboard() {
  ...
  const role = (profile?.role ?? 'business_analyst') as keyof typeof ROLE_LABELS
  const firstName = profile?.full_name?.split(' ')[0]
  const roleLabel = ROLE_LABELS[role] ?? 'Business Analyst'
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-gray-500">
          Dig into startup data and surface the insights that matter.
        </p>
      </div>

      {/* Unique analyst hero */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-700 via-slate-800 to-gray-900 p-8 text-white">
        <div className="relative z-10">
          ...
          <h2 className="mt-3 text-xl font-bold sm:text-2xl">Turn startup data into decisions</h2>
          <p className="mt-2 max-w-lg text-white/80">
            Analyze markets, model financials and write the reports founders act on.
          </p>
          <div className="mt-5 flex flex-wrap gap-3">
            <Link
              to="/explore"
              className="inline-flex items-center gap-2 rounded-lg bg-white px-5 py-2.5 text-sm font-bold text-slate-800 transition-transform hover:scale-105"
            >
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-dark-300 dark:bg-dark-100">
          ...
          <p className="text-sm text-gray-500">Startups to analyze</p>
        </div>
        <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-dark-300 dark:bg-dark-100">
          <MessagesSquare className="h-6 w-6 text-slate-700" />
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <section className="lg:col-span-2">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="font-bold">Startups worth analyzing</h2>
            ...
            {loadingRec
              ...
              : recommended.length === 0 ? (
                  <div className="rounded-2xl border border-dashed border-gray-300 p-8 text-center dark:border-dark-400 sm:col-span-2">
                    ...
                    <p className="mt-3 font-semibold">No data to analyze yet</p>
